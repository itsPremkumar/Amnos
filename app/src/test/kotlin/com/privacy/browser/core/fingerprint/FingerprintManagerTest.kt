...
class FingerprintManagerTest {
    ...
    fun profileGenerationIsDeterministicPerSessionAndTab() {
        val first = FingerprintManager.generateCoherentProfile("session-1", "tab-1")
        val second = FingerprintManager.generateCoherentProfile("session-1", "tab-1")
    ...
    fun differentTabsReceiveDistinctTabScopedNoise() {
        val first = FingerprintManager.generateCoherentProfile("session-1", "tab-1")
        val second = FingerprintManager.generateCoherentProfile("session-1", "tab-2")
    ...
    fun generatedProfilesStayWithinAndroidMobileShape() {
        val profile = FingerprintManager.generateCoherentProfile("session-android", "tab-android")
